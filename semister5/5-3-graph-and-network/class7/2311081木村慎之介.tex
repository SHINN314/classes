...
\begin{prop}
頂点数\(3\)以上のグラフ\(G\)が\(2\)-連結であるならば、任意の\(2\)頂点\(u, v\)が同じ閉路に含まれる。
\end{prop}

\begin{proof}[証明]
\(G\)は\(2\)-連結なグラフであるから、任意の\(2\)頂点を分離するには少なくとも選んだ頂点とは別の\(2\)つの頂点を取り除く必要がある。
従ってmengerの定理より、任意の\(2\)頂点\(u, v\)の間には少なくとも\(2\)つの点素な道が存在する。
この\(2\)つの道をおつなげれば\(2\)頂点を含む閉路を作ることができる。
\end{proof}
